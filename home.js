...
function noResult() {
  ...
  noResult.textContent = 'Nessun prodotto trovato.';
  container.appendChild(noResult);
}

function search(event) {
  const form_data = new FormData(document.querySelector("#search form"));
  fetch("search_content.php?q=" + encodeURIComponent(form_data.get('search'))).then(searchResponse).then(jsonProdotti).catch(noResult);
...
function searchMobile(event) {
  const form_data = new FormData(document.querySelector("#search-mobile form"));
  fetch("search_content.php?q=" + encodeURIComponent(form_data.get('search'))).then(searchResponse).then(jsonProdotti).catch(noResult);
  event.preventDefault();
}

document.querySelector("#search-mobile form").addEventListener('submit', searchMobile);
